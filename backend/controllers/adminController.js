...
const updateRole = async (req, res) => {
  ...
  // Prevent updating the admin role (id=1)
  if (id === "1") {
    return res.status(403).json({ message: "Cannot modify the Admin role" });
  }
  ...
  try {
    ...
    if (parseInt(usersWithRole.rows[0].count) > 0) {
      return res.status(400).json({
        message:
          "Cannot delete role as it is assigned to users. Reassign users first.",
      });
    }
